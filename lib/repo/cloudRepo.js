...
export function createCloudRepo(supabase, userId) {
  ...
  return {
    ...
    // Retry-safe language write. The profiles row is created by the auth
    // trigger a moment AFTER sign-up, so a plain update can land on a row that
    // doesn't exist yet (0 rows updated, no error → silently lost). We update
    // WITHOUT .single() so 0 rows returns [] instead of throwing, then wait and
    // retry until the row exists and the write actually takes.
    async setAppLanguage(lang, opts = {}) {
      const retries = opts.retries ?? 6;
      const delayMs = opts.delayMs ?? 400;
      for (let i = 0; i <= retries; i++) {
        const { data, error } = await supabase
          .from("profiles")
          .update({ app_language: lang, updated_at: new Date().toISOString() })
          .eq("id", uid)
          .select("id, app_language"); // no .single() → [] when row not ready
        if (error) throw error;                 // a real error, surface it
        if (data && data.length > 0) return data[0]; // row existed, write took
        await new Promise((r) => setTimeout(r, delayMs)); // not ready, wait
      }
      throw new Error("profile row not available for language update after retries");
    },
    ...
    async recordDecision(input) {
      const row = {
        ...
        allocated_amount: input.allocated_amount ?? null,
      };
